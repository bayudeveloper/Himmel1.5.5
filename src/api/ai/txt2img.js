...
const nanoHeaders = {
    ...
    'referer': 'https://www.nanobana.net/m/txt2img'
};
...
async function login(cookieStore, email) {
    ...
    const page = await axios.get('https://www.nanobana.net/m/txt2img', {
        headers: nanoHeaders,
        timeout: 15000
    });
...
async function verifyOtp(cookieStore, email, code) {
    ...
    // Login dengan OTP
    const data = `email=${encodeURIComponent(email)}&code=${code}&redirect=false&csrfToken=${csrfToken}&callbackUrl=${encodeURIComponent('https://www.nanobana.net/m/txt2img')}`;
...
async function checkStatus(cookieStore, taskId, prompt) {
    const endpoints = [
        ...
        `https://www.nanobana.net/api/txt2img/task/${taskId}?prompt=${encodeURIComponent(prompt)}`,
