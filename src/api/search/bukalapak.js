...
async function scrapeBukalapak(search) {
  try {
    ...
    $(".bl-product-card").each((i, el) => {
      ...
      const title = $(el)
        .find(".bl-product-card__description-name")
        .text()
        .trim();

      const price = $(el)
        .find(".bl-product-card__description-price")
        .text()
        .trim();

      const rating = $(el)
        .find(".bl-product-card__description-rating")
        .text()
        .trim();
...
/* ===========================
   ENDPOINT
=========================== */
app.get("/bukalapak", async (req, res) => {
...
app.get("/", (req, res) => {
  res.json({
    message: "Bukalapak Scraper API Ready 🚀",
    usage: "/bukalapak?search=laptop"
  });
});
